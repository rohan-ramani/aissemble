...
public class MlPipelineDockerPomMigration extends AbstractPomMigration {
    ...
    protected boolean shouldExecuteOnFile(File file) {
        ...
        if (model.getPackaging().equals("docker-build")) {
            ...
            return stepArtifactId != null && model.getDependencies().stream()
                    .noneMatch(dependency -> isMlStep(dependency, stepArtifactId));
        }
        return false;
    }
    ...
    private String getMlPipelineStepArtifact(File file) {
        try {
            List<String> stepArtifactIds = getStepArtifactIds();
            List<String> lines = Files.readAllLines(file.toPath());
            for (String line: lines) {
                line = line.trim();
                if (line.matches("<outputDirectory>.*</outputDirectory>")) {
                    var o = stepArtifactIds.stream().filter(line::contains).findFirst();
                    if(o.isPresent()) {
                        return o.get();
                    }
                }
