...
| Property                                            | Description                                                                                   | Required Override | Default             |
|-----------------------------------------------------|-----------------------------------------------------------------------------------------------|-------------------|---------------------|
| persistentVolumeProperties.enabled                  | Enables or disables the volume generation                                                     | No                | false               |
| persistentVolumeProperties.name                     | The name used to create the pv and pvc                                                        | No                | configuration-store |
| persistentVolumeProperties.storageType              | Determines mounting a local mount for tests or a custom mount <br/>Options: local, custom     | No                | custom              |
| persistentVolumeProperties.storageClass             | The PVCs class of storage. If storageType is custom then the storageClass needs to be defined | No*               | ""                  |
| persistentVolumeProperties.volumePathOnNode         | Defines the host machine filesystem path to mount                                             | Yes**             | ""                  |
| persistentVolumeProperties.accessModes.ReadOnlyMany | The modes this PVC will support when mounting                                                 | No                | ReadOnlyMany        |
| persistentVolumeProperties.size                     | The size of the PVC                                                                           | No                | 1Gi                 |     
...
The Configuration Store also support a simplified way of providing configurations without mounting properties files.

The following properties are used to create a configMap for defining and mounting configuration properties files.

| Property                                   | Description                                              | Required Override | Default                    |
|--------------------------------------------|----------------------------------------------------------|-------------------|----------------------------|
| configMapProperties.enabled         | Enables mounting properties files from configMaps        | No                | true                       |
| configMapProperties.name            | The name of Configuration properties configMap           | No                | configuration-store-config |
| configMapProperties.baseConfigFiles | The list of base config properties files to mount        | No                | spark.properties:          |
| configMapProperties.envConfigFiles  | The list of environment config properties files to mount | No                | spark.properties:          |


#### Defining Properties Files
The user can define its configuration properties within their values file as below:
