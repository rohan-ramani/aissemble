...
=== For Local Cluster Testing via Helmfile

After deploying with Helmfile, all applications except the pipeline will be deployed. Once all other resources are ready, you can 
deploy the pipeline(s) with `helmfile apply --state-values-set pipeline=<pipeline-name>` to submit the associated `SparkApplication` to the cluster.

To view logging for the pipeline, you can run `kubectl logs <pipeline-name>-driver -n <project-name>` to view the driver logs.
