...
* Java: https://adoptium.net/temurin/archive/?version=21[JDK 21,role=external,window=_blank]
...
* Continuous Deployment: https://helmfile.readthedocs.io/en/latest/#installation[Helmfile,role=external,window=_blank] for local development
** For the `helmfile apply` command to work users will also need to install the `helm-diff` plugin with the following command:
+
[source, bash]
----
